...
def draw_dashboard_preview() -> None:
    ...
    draw.rectangle((24, 24, 264, 790), fill="#ffffff", outline="#d0d7de")
    ...
    for idx, label in enumerate(
        ...
        draw.rounded_rectangle((44, y, 234, y + 42), radius=8, fill=color, outline="#c9d3df")
    ...
    panels = [
        (304, 44, 642, 342, "Market Data"),
        (660, 44, 988, 342, "Training"),
        (304, 364, 988, 742, "Backtest"),
    ]
    ...
    market_points = [
        (326 + int(x * 300), 310 - int(y * 170)) for x, y in zip(x_values, market, strict=True)
    ]
    reward_points = [
        (682 + int(x * 280), 310 - int(y * 170)) for x, y in zip(x_values, reward, strict=True)
    ]
    backtest_points_a = [
        (326 + int(x * 630), 690 - int(y * 260)) for x, y in zip(x_values, backtest_a, strict=True)
    ]
    backtest_points_b = [
        (326 + int(x * 630), 690 - int(y * 260)) for x, y in zip(x_values, backtest_b, strict=True)
    ]
    ...
    draw.text((326, 96), "Close price", fill="#4b5563", font=font(15))
    draw.text((682, 96), "Reward + mean loss", fill="#4b5563", font=font(15))
    draw.text((326, 414), "DQN vs Buy-and-Hold equity", fill="#4b5563", font=font(15))
